...
fn main() {
    ...
    match cfgs.validate() {
        ...
        CameraConfigurationStatus::Invalid => panic!("Error validating camera_wrapper configuration"),
    }
    ...
    println!("Waiting for camera_wrapper request execution");
    ...
    let img =  match image::load_from_memory_with_format(&jpeg_data[..jpeg_len], image::ImageFormat::Jpeg) {
        Ok(di) => di,
        Err(_) => panic!("Can't load JPEG")
    };
    let gray = img.grayscale();
    gray.save_with_format("./gray.png", ImageFormat::Png).unwrap();
