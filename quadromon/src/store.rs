...
impl Into<StoreItem> for ReadResult {
    fn into(self) -> StoreItem {
        ...
        StoreItem {
            ...
            time_stamp: SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap(),
            value: v
        }
    }
}

pub(crate) fn store_result(data: Vec<ReadResult>) -> Vec<StoreItem> {
    let items = data.into_iter().map(|r| r.into()).collect();
    store(&items);
    items
}

pub(crate) fn store(data: &Vec<StoreItem>) {
    let store_dir = dirs::data_local_dir().unwrap_or(dirs::home_dir().expect("Failed to determine home directory.")).join(APP_NAME);
    if !store_dir.exists() {
    ...
    let mut date_vec = serde_json::from_str::<Vec<StoreItem>>(&current).expect("Failed to parse store file");
    for i in data {
        let item = i.clone();
        date_vec.push(item);
    }
...
pub (crate) fn load() -> Result<Vec<StoreItem>, String> {
    ...
    let mut items: Vec<StoreItem> = match serde_json::from_slice(&stored_text).map_err(|e| format!("Failed to parse store file: {}", e)) {
        Ok(r) => r,
        Err(e) => return Err(e),
    };
