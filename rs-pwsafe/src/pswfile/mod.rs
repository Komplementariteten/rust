...
use crate::PswSafeError::{EofPositionError, FileNotSupported, FileToSmall, IterationsNotInitialized, NumberOfIterationsNotFound, SaltNotFound};
...
impl PswSafe {
    ...
    pub fn check_key(&self, mut pw: Vec<u8>) -> Result<bool, PswSafeError> {
        let key = self.get_stretch_key(pw)?;
