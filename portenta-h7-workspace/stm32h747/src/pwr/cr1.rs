...
pub type FLPS_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
...
pub type SVOS_W<'a, REG, const O: u8> = crate::FieldWriter<'a, REG, 2, O>;
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn flps(&mut self) -> FLPS_W<CR1_SPEC, 9> {
        FLPS_W::new(self)
    }
    #[doc = "Bits 14:15 - System Stop mode voltage scaling selection These bits control the VCORE voltage level in system Stop mode, to obtain the best trade-off between power consumption and performance."]
    #[inline(always)]
    #[must_use]
    pub fn svos(&mut self) -> SVOS_W<CR1_SPEC, 14> {
        SVOS_W::new(self)
    }
    #[doc = "Bit 16 - Peripheral voltage monitor on VDDA enable"]
    #[inline(always)]
    #[must_use]
    pub fn avden(&mut self) -> AVDEN_W<CR1_SPEC, 16> {
        AVDEN_W::new(self)
    }
    #[doc = "Bits 17:18 - Analog voltage detector level selection These bits select the voltage threshold detected by the AVD."]
    #[inline(always)]
    #[must_use]
    pub fn als(&mut self) -> ALS_W<CR1_SPEC, 17> {
        ALS_W::new(self)
    }
