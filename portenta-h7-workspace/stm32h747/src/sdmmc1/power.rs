...
pub type VSWITCH_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
...
pub type VSWITCHEN_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
...
pub type DIRPOL_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
impl R {
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn vswitch(&mut self) -> VSWITCH_W<POWER_SPEC, 2> {
        VSWITCH_W::new(self)
    }
    #[doc = "Bit 3 - Voltage switch procedure enable. This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). This bit is used to stop the SDMMC_CK after the voltage switch command response:"]
    #[inline(always)]
    #[must_use]
    pub fn vswitchen(&mut self) -> VSWITCHEN_W<POWER_SPEC, 3> {
        VSWITCHEN_W::new(self)
    }
    #[doc = "Bit 4 - Data and command direction signals polarity selection. This bit can only be written when the SDMMC is in the power-off state (PWRCTRL = 00)."]
    #[inline(always)]
    #[must_use]
    pub fn dirpol(&mut self) -> DIRPOL_W<POWER_SPEC, 4> {
        DIRPOL_W::new(self)
    }
