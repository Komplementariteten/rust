...
pub fn draw<B: Backend>(f: &mut Frame<B>, app: &mut App) {
    let chunks = Layout::default()
        ...
        .split(f.size());
    let menuItems: Vec<ListItem> = app
        .menu
        .items
        .iter()
        // .map(|t| Spans::from(Span::styled(*t, Style::default().fg(Color::Green))))
        .map(|t| ListItem::new(Text::styled(*t, Style::default().fg(Color::White))))
        .collect();
    let list = List::new(menuItems)
        .block(Block::default().borders(Borders::ALL).title(app.title))
        .highlight_style(Style::default().fg(Color::LightCyan))
        .highlight_symbol("💡");
    f.render_stateful_widget(list, chunks[0], &mut app.menu.state);
    match app.menu.state.selected() {
        Some(0) => draw_datastore(f, app, chunks[1]),
        Some(1) => draw_second_tab(f, app, chunks[1]),
        Some(2) => draw_third_tab(f, app, chunks[1]),
        _ => {}
    };
}

fn draw_datastore<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
    B: Backend,
{
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([Constraint::Ratio(1, 7), Constraint::Ratio(6, 7)].as_ref())
        .split(area);
    draw_input(f, app, chunks[0]);
    if app.should_submit && app.context == InputContext::DataStorePath {
        if app.data_store.is_none() {
            let p = Path::new(app.ds_base_path.as_str());
            app.data_store = Some(Datastore::new(p.to_path_buf()));
            println!("datastore created");
        }
    }

    draw_storeview(f, app, chunks[1]);
    // draw_text(f, chunks[1]);
}

fn draw_storeview<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
    B: Backend,
{
    let chunks = Layout::default()
